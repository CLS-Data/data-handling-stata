...
In this section, we show a few `Stata` commands for exploring NCDS data;
as noted, historical sweeps of the NCDS did not use modern metadata
...
`N622`, for example.) In what follows, we will use the `Stata` commands
to find variables for cohort members’ height, which has been collected
in many of the sweeps.
...
Below, we read in the NCDS 55-year sweep dataset which contains derived
variables (`55y/ncds_2013_derived.dta`) and use `lookfor` to search for
variables related to `"height"`.

``` stata
use "55y/ncds_2013_derived.dta", clear
lookfor "height"
```
    ...
    --------------------------------------------------------------------------------
    ND9HGHTM        double  %12.0g     ND9HGHTM   (Derived) Height in metres
...
creation of data dictionaries. Below we create a codebook for the NCDS
55-year sweep dataset (the open dataset), saving it in a log file.

    log using codebook.log, replace
        codebook
    log close

![Codebook printed in log file](../images/ncds-data_discovery.png)
...
all the `.dta` files in the NCDS folder.
